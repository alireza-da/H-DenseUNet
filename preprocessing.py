...
def generate_livertxt(image_path, save_folder):
    ...
    for i in range(0, 131):
        ...
        f = open('data/' + save_folder +
                 '/LiverPixels/liver_' + str(i) + '.txt', 'w')
...
def generate_txt(image_path, save_folder):
    ...
    for i in range(0, 131):
        values = np.loadtxt('data/myTrainingDataTxt/LiverPixels/liver_' +
                            str(i) + '.txt', delimiter=' ', usecols=[0, 1, 2])
        a = np.min(values, axis=0)
        b = np.max(values, axis=0)
        box = np.append(a, b, axis=0)
        np.savetxt('data/myTrainingDataTxt/LiverBox/box_' +
                   str(i)+'.txt', box, fmt='%d')
...
generate_livertxt(image_path='data/TrainingData/',
                  save_folder='myTrainingDataTxt/')
print("Generate tumor txt")
generate_tumortxt(image_path='data/TrainingData/',
                  save_folder='myTrainingDataTxt/')
